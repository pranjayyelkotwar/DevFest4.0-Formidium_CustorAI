...
interface EmailResponseModalProps {
  ...
  onCloseIssue: (email_id: string) => Promise<void>; // Add this prop
}

export function EmailResponseModal({ email, onClose, onSend, onCloseIssue }: EmailResponseModalProps) {
  const [body, setBody] = useState('');
  const [sending, setSending] = useState(false);
  const [closing, setClosing] = useState(false); // State for closing issue
  ...
  const handleCloseIssue = async () => {
    try {
      setClosing(true);
      await onCloseIssue(email.email_id); // Call the onCloseIssue function
      onClose();
    } catch (error) {
